...
class BaristaGUI(Frame):
    ...
    def init_components(self):
        ...
        self.columnFemalees = self.baristaBLL.getBaristaDAL().getColumnFemalees()
        ...
        self.table = ttk.Treeview(self.tableFrame, columns=self.columnFemalees[0:len(self.columnFemalees)-1], show="headings")
        self.table.heading("0", text="BARISTA_ID")
        ...
        self.data = self.baristaBLL.getData()
        ...
        self.table.bind('<<TreeviewSelect>>', self.showDetailsBarista)

        self.pnlBaristaConfiguration = Frame(self.panel2, bg="#FFFFFF", width=450)
        self.pnlBaristaConfiguration.pack(padx=10, pady=10)

        self.labelForm = []
        self.TextFieldsForm = []
        self.row = 0
        self.column = 0
        for i in range(0, len(self.columnFemalees)-1):
            self.labelForm.append(Label(self.pnlBaristaConfiguration, text=self.columnFemalees[i] + ": ", fg="#000000", bg="#ffffff"))
            self.labelForm[i].grid(row=self.row, column=self.column, padx=20, pady=10)
            self.column = self.column + 1
            if self.columnFemalees[i] == "BARISTA_ID":
                self.TextFieldsForm.append(Entry(self.pnlBaristaConfiguration, fg="#000000", bg="#ffffff", width=30))
                self.TextFieldsForm[len(self.TextFieldsForm)-1].insert(0, self.baristaBLL.getAutoID())
            ...
            elif self.columnFemalees[i] == "GENDER":
                self.cbbGender = ttk.Combobox(self.pnlBaristaConfiguration, values=["Female", "Male"], width=27)
        ...
        self.btAdd = Button(self.mode, text="ADD", width=15, bg="#673c16", state="normal", command=self.add)
        self.btAdd.grid(row=0, column=0, padx=20, pady=10, ipady=4)

        self.btUpd = Button(self.mode, text="UPDATE", width=15, bg="#673c16", state="disabled", command=self.upd)
        self.btUpd.grid(row=0, column=1, padx=20, pady=10, ipady=4)

        self.btDel = Button(self.mode, text="DELETE", width=15, bg="#673c16", state="disabled", command=self.dele)
        self.btDel.grid(row=1, column=0, padx=20, pady=10, ipady=4)

        self.btRef = Button(self.mode, text="Refresh", width=15, bg="#673c16", state="normal", command=self.ref)
        self.btRef.grid(row=1, column=1, padx=20, pady=10, ipady=4)

    def showDetailsBarista(self, event):
    ...
    def add(self):
        gender = True if self.cbbGender.get() == "Female" else False
        dob = datetime.strptime(self.TextFieldsForm[2].get(), "%Y-%m-%d").date()
        doe = datetime.strptime(self.TextFieldsForm[7].get(), "%Y-%m-%d").date()
        self.newBarista = Barista(self.TextFieldsForm[0].get(), self.TextFieldsForm[1].get(), gender, dob, self.TextFieldsForm[3].get(), self.TextFieldsForm[4].get(), self.TextFieldsForm[5].get(), float(self.TextFieldsForm[6].get()), doe, False)
        self.baristaBLL.addBarista(self.newBarista)
        self.ref()
    ...
    def upd(self):
        ...
        doe = datetime.strptime(self.TextFieldsForm[7].get(), "%Y-%m-%d").date()
        self.newBarista = Barista(self.TextFieldsForm[0].get(), self.TextFieldsForm[1].get(), gender, dob, self.TextFieldsForm[3].get(), self.TextFieldsForm[4].get(), self.TextFieldsForm[5].get(), float(self.TextFieldsForm[6].get()), doe, False)
        self.baristaBLL.updateBarista(self.newBarista)
        self.loadTable()

    def dele(self):
        self.baristaBLL.deleteBarista(Barista(baristaID = self.TextFieldsForm[0].get()))
        self.ref()
    ...
    def ref(self):
        ...
        self.TextFieldsForm[0].configure(state="normal")
        self.TextFieldsForm[0].delete(0, END)
        self.TextFieldsForm[0].insert(END, self.baristaBLL.getAutoID())
        self.TextFieldsForm[0].configure(state="readonly")

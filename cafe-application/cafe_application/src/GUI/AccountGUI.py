...
class AccountGUI(Frame):
    ...
    def init_components(self):
        ...
        self.table.heading("4", text="BARISTA_ID")
        ...
        for i in range(0, len(self.columnNames)-1):
            ...
            if self.columnNames[i] == "ACCOUNT_ID":
                ...
                self.cbbbaristaID = ttk.Combobox(self.pnlAccountConfiguration, values=self.baristasID, width=27)
                self.cbbbaristaID.configure(state="readonly")
                self.cbbbaristaID.grid(row=self.row, column=self.column, padx=10, pady=10, ipady=4)
    ...
    def showDetailsAccount(self, event):
        for row in self.table.selection():
            ...
            self.cbbbaristaID.set(values[4])
            self.cbbbaristaID.configure(state="readonly")
    ...
    def add(self):
        self.newAccount = Account(self.TextFieldsForm[0].get(), self.TextFieldsForm[1].get(), self.TextFieldsForm[2].get(), self.cbbDecentralizationID.get(), self.cbbbaristaID.get(), False)

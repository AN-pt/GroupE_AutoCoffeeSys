...
class DecentralizationGUI(Frame):
    ...
    def init_components(self):
        ...
        for i in range(0, len(self.columnNames)-1):
            ...
            if self.columnNames[i] == "DECENTRALIZATION_ID":
                ...
                self.cbbForm.append(ttk.Combobox(self.frameInCanvas, values=["Remove", "View", "Edit", "View and Edit"], width=24))
    ...
    def add(self):
        self.arr = []
        for i in range(0,len(self.cbbForm)):
            if self.cbbForm[i].get() == "Remove":
                self.arr.append(0)
            if self.cbbForm[i].get() == "View":
                self.arr.append(1)
            if self.cbbForm[i].get() == "Edit":
                self.arr.append(2)
            if self.cbbForm[i].get() == "View and Edit":
                self.arr.append(3)

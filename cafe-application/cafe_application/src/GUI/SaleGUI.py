...
class SaleGUI(Frame):
    ...
    def init_components(self):
        self.panel1 = Frame(self, bg="#673c16", width=680)
        ...
        self.baristasID = []
        for barista in self.baristaBLL.getBaristaList():
            self.baristasID.append(self.baristaBLL.getValueByKey(barista, "BARISTA_ID"))

        self.tableFrame = Frame(self.panel1, width=660, height=720, bg="#673c16", highlightthickness=0, borderwidth=0)
        ...
        for product in self.productsList:
            ...
            self.img.append(ImageTk.PhotoImage(Image.open(product.getImage()).resize((175, 150))))
            ...
            self.productsName.append(Label(self.frameProducts[len(self.frameProducts)-1], font=("calibri", 9, "bold"), text=product.getName() + "-" + product.getProductID(), fg="#000000", bg="#ffffff"))
            self.productsName[len(self.productsName)-1].grid(row = 1, column = 0)

            self.productsCost.append(Label(self.frameProducts[len(self.frameProducts)-1], font=("calibri", 10, "bold"), text=product.getCost(), fg="#000000", bg="#ffffff"))
        ...
        for i in range(0, len(self.columnNames)-1):
            ...
            elif self.columnNames[i] == "BARISTA_ID":
                self.cbbBaristaID = ttk.Combobox(self.pnlBillConfiguration, values=self.baristasID, width=27)
                self.cbbBaristaID.configure(state="readonly")
                self.cbbBaristaID.grid(row=self.row, column=self.column, padx=20, pady=10, ipady=4)
        ...
        self.btBuy = Button(self.mode, text="BUY", width=15, bg="#673c16", state="normal", command=self.buy)
        self.btBuy.grid(row=0, column=0, padx=20, pady=10, ipady=4)

        self.detection = Detection(self.TextFieldsForm)
        self.btDec = Button(self.mode, text="DETECTION", width=15, bg="#673c16", state="normal", command=self.dec)
        self.btDec.grid(row=0, column=1, padx=20, pady=10, ipady=4)


    def on_panel_click(self, event):
        self.message = messagebox.askyesno("Confirm Purchase", "Are you sure you would like to buy this product?")
        if self.message:
            ...
            for item in  self.TextFieldsBillDetails:
                if item.get() == self.productsList[index].getProductID():
                    messagebox.showinfo("Message", "Your product has been bought successfully!")
                    return
    ...
    def buy(self):
        ...
        self.billBLL.addBill(Bill(self.TextFieldsForm[0].get(), name, self.cbbBaristaID.get(), dop, float(0), False))
        for i in range(0, len(self.TextFieldsBillDetails)):
            if i % 2 == 0:
                product = self.productBLL.findProductsBy({"PRODUCT_ID": self.TextFieldsBillDetails[i].get()})[0]
            else:
    ...
    def ref(self):
        ...
        self.cbbBaristaID.configure(state="normal")
        self.cbbBaristaID.delete(0, END)
        self.cbbBaristaID.configure(state="readonly")

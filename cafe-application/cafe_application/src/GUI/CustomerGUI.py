...
class CustomerGUI(Frame):
    ...
    def init_components(self):
        ...
        self.columnFes = self.customerBLL.getCustomerDAL().getColumnFes()
        ...
        self.table = ttk.Treeview(self.tableFrame, columns=self.columnFes[0:len(self.columnFes)-1], show="headings")
        ...
        self.row = 0
        self.column = 0
        for i in range(0, len(self.columnFes)-1):
            self.labelForm.append(Label(self.pnlCustomerConfiguration, text=self.columnFes[i] + ": ", fg="#000000", bg="#ffffff"))
            self.labelForm[i].grid(row=self.row, column=self.column, padx=20, pady=10)
            self.column = self.column + 1
            if self.columnFes[i] == "CUSTOMER_ID":
            ...
            elif self.columnFes[i] == "GENDER":
                self.cbbGender = ttk.Combobox(self.pnlCustomerConfiguration, values=["F", "M"], width=27)
                self.cbbGender.configure(state="readonly")
                self.cbbGender.grid(row=self.row, column=self.column, padx=20, pady=10, ipady=4)
            elif self.columnFes[i] == "MEMBERSHIP":
                self.cbbMembership = ttk.Combobox(self.pnlCustomerConfiguration, values=["Available", "Unavailable"], width=27)
        ...
        self.btDetection = Button(self.search, text="Facial recognition search", width=25, bg="#e6f5f3", command=self.findByFace)
        self.btDetection.pack(pady=5)


        self.btRecord = Button(self.pnlCustomerConfiguration, text="Face registration", width=25, bg="#e6f5f3", command=self.sinupFace, state="disabled")
    ...
    def add(self):
        gender = True if self.cbbGender.get() == "F" else False
        member = True if self.cbbMembership.get() == "M" else False

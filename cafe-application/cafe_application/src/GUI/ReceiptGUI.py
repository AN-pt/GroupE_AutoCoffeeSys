...
class ReceiptGUI(Frame):
    ...
    def init_components(self):
        ...
        self.table.heading("1", text="BARISTA_ID")
        ...
        for i in range(0, len(self.columnNames)-1):
            ...
            if self.columnNames[i] == "RECEIPT_ID":
                ...
                self.cbbBaristaID = ttk.Combobox(self.pnlReceiptConfiguration, values=self.baristasID, width=27)
        ...
        self.btAdd = Button(self.mode, text="ADD", width=15, bg="#673c16", fg="#ffffff", state="normal", command=self.add)
        self.btAdd.grid(row=0, column=0, padx=20, pady=10, ipady=4)

        self.btUpd = Button(self.mode, text="UPDATE", width=15, bg="#673c16", fg="#ffffff", state="disabled", command=self.upd)
        self.btUpd.grid(row=0, column=1, padx=20, pady=10, ipady=4)

        self.btDel = Button(self.mode, text="DELETE", width=15, bg="#673c16", fg="#ffffff", state="disabled", command=self.dele)
        self.btDel.grid(row=1, column=0, padx=20, pady=10, ipady=4)

        self.btRef = Button(self.mode, text="Refresh", width=15, bg="#673c16", fg="#ffffff", state="normal", command=self.ref)
    ...
    def showDetailsReceipt(self, event):
        for row in self.table.selection():
            ...
            self.cbbBaristaID.set(values[1])
            self.cbbBaristaID.configure(state="readonly")
    ...
    def add(self):
        ...
        self.newReceipt = Receipt(self.TextFieldsForm[0].get(), self.cbbBaristaID.get(), dor, float(self.TextFieldsForm[2].get()), False)
